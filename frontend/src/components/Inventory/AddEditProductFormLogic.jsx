...
const defaultNewProductState = {
    ...
    stock: 0, // Cambiado a número para evitar problemas de validación
    costPrice: 0, // Cambiado a número para evitar problemas de validación
    ...
    profitPercentage: 30, // Valor por defecto numérico
...
const AddEditProductFormLogic = ({
    ...
    useEffect(() => {
        if (isOpen) {
            if (isNewProduct) {
                ...
                // Resetear estados de SKU para nuevo producto
                setAutoGeneratedSku('');
    ...
    const validateField = useCallback((name, value, currentProductState, variantIndex = null) => {
        ...
        if (
            ...
            if (name === 'costPrice' && (value === undefined || value === null || isNaN(Number(value)) || Number(value) < 0)) {
                error = 'El costo unitario debe ser un número no negativo.';
            } else if (name === 'stock' && (value === undefined || value === null || isNaN(Number(value)) || Number(value) < 0)) {
                error = 'El stock debe ser un número no negativo.';
            } else if (name === 'profitPercentage' && value !== undefined && value !== null && value !== '' && (isNaN(Number(value)) || Number(value) < 0 || Number(value) > 500)) {
    ...
    const handleProductInputChange = useCallback((e) => {
        ...
        if (name === 'name' && isNewProduct && !isMainSkuManuallyEdited) {
            const newSku = generateSkuFromName(inputValue);
            setAutoGeneratedSku(newSku);
            // Actualizar también el SKU en productData si no ha sido editado manualmente
            setProductData(prev => ({
                ...prev,
                name: inputValue,
                sku: newSku
        ...
        // Búsqueda de productos globales al escribir el nombre
        if (name === 'name' && isNewProduct && inputValue.length >= 3) {
            if (debounceTimeoutRef.current) {
                clearTimeout(debounceTimeoutRef.current);
            }
            debounceTimeoutRef.current = setTimeout(() => {
                searchGlobalProducts(inputValue);
            }, 500);
        } else if (name === 'name' && inputValue.length < 3) {
            setGlobalProductSuggestions([]);
            setShowGlobalSuggestions(false);
            setNoGlobalSuggestionsFound(false);
        }
        ...
        setProductData(prev => {
            ...
            if (['costPrice', 'stock', 'reorderThreshold', 'optimalMaxStock', 'shelfLifeDays', 'profitPercentage'].includes(name)) {
                // Convertir a número, usar 0 como fallback para campos vacíos
                updatedData[name] = inputValue === '' ? 0 : Number(inputValue) || 0;
            } else {
            ...
            if (name === 'sku') {
                if (String(inputValue).trim() !== '') {
                    ...
                    setIsMainSkuManuallyEdited(false);
                    // Si se borra el SKU, regenerar automáticamente basado en el nombre
                    if (isNewProduct && updatedData.name) {
                        const newSku = generateSkuFromName(updatedData.name);
    ...
    const handleVariantInputChange = useCallback((index, e) => {
        ...
        setProductData(prev => {
            ...
            if (['costPrice', 'stock', 'reorderThreshold', 'optimalMaxStock', 'shelfLifeDays', 'profitPercentage'].includes(name)) {
                // Manejar campos numéricos correctamente
                updatedVariant[name] = inputValue === '' ? 0 : Number(inputValue) || 0;
            } else {
    ...
    const handleAddVariant = useCallback(() => {
        ...
        const newVariant = {
            name: '', sku: '', costPrice: 0, stock: 0, // Valores numéricos por defecto
            ...
            profitPercentage: defaultProfitForNewVariant, // Usar el valor calculado
        };
    ...
    // Función para buscar productos en el catálogo global
    const searchGlobalProducts = useCallback(async (searchTerm) => {
        ...
        try {
            const response = await axiosInstance.get(`/products/global-search?q=${encodeURIComponent(searchTerm.trim())}`);
            const suggestions = response.data || [];
            
            if (suggestions.length > 0) {
                setGlobalProductSuggestions(suggestions.slice(0, 5)); // Limitar a 5 sugerencias
            ...
            console.error('Error al buscar productos globales:', err);
            setGlobalProductSuggestions([]);
            setShowGlobalSuggestions(false);
            setNoGlobalSuggestionsFound(false);
        }
    ...
    const handleProductSelect = (selectedProduct) => {
        ...
        const newData = {
            ...
            costPrice: 0,
            stock: 0,
            _isFromGlobalProduct: true, // Marcar que viene de producto global
    ...
    const handleNewProduct = (productName) => {
        const newSku = generateSkuFromName(productName);
        setProductData({
        ...
        <ErrorBoundary>
            {currentStep === 'search' ? (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className={`rounded-2xl shadow-2xl w-full max-w-2xl relative overflow-hidden ${
                        theme === 'light' 
