...
const AddEditProductFormLogic = ({
    ...
    const { exchangeRate, loadingCurrency, currencyError, fetchExchangeRate, convertPrice, formatPrice, baseCurrency: contextBaseCurrency, availableCurrencies } = useCurrency();
    ...
    useEffect(() => {
        ...
        if (loadingCurrency || !exchangeRate) {
            return;
        }
    ...
    const handleProductInputChange = useCallback((e) => {
        ...
        setProductData(prev => {
            ...
            if (['costPrice', 'stock', 'reorderThreshold', 'optimalMaxStock', 'shelfLifeDays', 'profitPercentage'].includes(name)) {
                updatedData[name] = Number(inputValue);
            } else {
    ...
    const handleVariantInputChange = useCallback((index, e) => {
        ...
        setProductData(prev => {
            ...
            if (['costPrice', 'stock', 'reorderThreshold', 'optimalMaxStock', 'shelfLifeDays', 'profitPercentage'].includes(name)) {
                updatedVariant[name] = Number(inputValue);
            } else {
    ...
    const handleAddVariant = useCallback(() => {
        ...
        const newVariant = {
            ...
            profitPercentage: defaultProfitForNewVariant,
        };
    ...
    const handleSubmit = useCallback(async (e) => {
        ...
        if (productDataForSubmission.variants.length === 0) {
            productDataForSubmission.price = calculatedProductPricePlaceholder !== null ? Number(calculatedProductPricePlaceholder) : 0;
            productDataForSubmission.profitPercentage = calculatedProductProfitPercentage !== null ? Number(calculatedProductProfitPercentage) : productDataForSubmission.profitPercentage;
        }

        productDataForSubmission.variants = productDataForSubmission.variants.map((v, idx) => {
            const updatedVariant = { ...v };
            updatedVariant.price = calculatedVariantPricePlaceholder[idx] !== null ? Number(calculatedVariantPricePlaceholder[idx]) : 0;
            updatedVariant.profitPercentage = calculatedVariantProfitPercentage[idx] !== null ? Number(calculatedVariantProfitPercentage[idx]) : updatedVariant.profitPercentage;
        ...
        if (isNewProduct && !isMainSkuManuallyEdited && String(productDataToSend.sku).trim() === '') {
