...
const InventoryAlerts = ({ lowStockAlerts, highStockAlerts }) => {
    ...
    if (!hasLowStockAlerts && !hasHighStockAlerts) {
        return (
            <div className="mb-6 p-4 rounded-lg bg-green-900 bg-opacity-20 text-green-300 border border-green-500 flex items-center gap-3 animate-fade-in-down">
                <Info size={24} className="flex-shrink-0" />
                <p className="text-sm font-semibold">¡Todos tus productos están en niveles de stock saludables! Buen trabajo.</p>
            </div>
        );
    }
