...
const InventoryPage = () => {
    ...
    // Manejador para el cambio de inputs del formulario de añadir
    ...
    const handleAddProduct = async (e) => {
        ...
        if (!newProduct.name || !newProduct.category || !newProduct.price || newProduct.stock === '') {
            displayMessage('Por favor, completa todos los campos obligatorios (nombre, categoría, precio, stock).', 'error');
            return;
        }
        if (isNaN(Number(newProduct.price)) || Number(newProduct.price) <= 0) {
            displayMessage('El precio debe ser un número positivo.', 'error');
            return;
        }
        ...
        try {
            ...
            setNewProduct({ name: '', description: '', category: '', price: '', stock: '' }); // Resetear formulario
    ...
    const handleEditClick = (product) => {
        setEditingProduct({ ...product, price: parseFloat(product.price).toFixed(2) }); // Formatea precio para el input
    ...
    // Manejador para el cambio de inputs del formulario de edición
    ...
    const handleUpdateProduct = async (e) => {
        ...
        // Validación básica en frontend para el producto editado
                    ...
                    <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                // CAMBIO AQUÍ: text-gray-900 para visibilidad
                    ...
                    {products.map((product) => (
                        <div key={product._id} className="bg-deep-night-blue p-6 rounded-lg shadow-lg border border-action-blue-light flex flex-col justify-between transform transition duration-300 hover:scale-[1.02] hover:shadow-2xl">
                            <div>
                                ...
                                <p className="text-neutral-light mb-1"><span className="font-semibold">Precio:</span> <span className="text-success-green font-bold text-lg">${parseFloat(product.price).toFixed(2)}</span></p>
